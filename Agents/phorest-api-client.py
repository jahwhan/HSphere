...
FAKE_CLIENT_NAMES = [
    "Max Mustermann", "Erika Musterfrau", "Hans Meyer", "Lisa Schmidt",
    "Thomas Weber", "Julia Fischer", "Michael Wagner", "Sarah Becker",
    "Klaus Hoffmann", "Anna Schulz"
]
...
def search_clients(query: str, limit: int = 10) -> Dict:
    ...
    try:
        if USE_FAKE_API:
            ...
            fake_clients = []
            for i, name in enumerate(random.sample(FAKE_CLIENT_NAMES, min(limit, len(FAKE_CLIENT_NAMES)))):
...
def get_available_slots(staff_id: str, service_id: str, date: str) -> Dict:
    ...
    try:
        if USE_FAKE_API:
            ...
            available_slots = random.sample(base_times, k=random.randint(4, len(base_times)))
